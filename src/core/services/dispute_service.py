...
class DisputeService:
    ...
    async def get_timeline(self, dispute_id: int) -> DisputeTimelineResponse:
        ...
        if inbound_email_ids:
            try:
                ...
                result = await self.db.execute(
                    ...
                    .join(
                        EmailMessageAttachment,
                        EmailMessageAttachment.message_id
        ...
        if outbound_episodes:
            try:
                ...
                result = await self.db.execute(
                    ...
                    .join(
                        OutboundEmailAttachment,
                        OutboundEmailAttachment.outbound_id
                        == OutboundEmail.outbound_id,
                    )
        ...
        if associate_episodes:
            try:
                ...
                logger.warning(
                    f"[dispute_id={dispute_id}] Failed to load FA actor names: {exc}"
                )
        ...
        if dispute_type_id:
            ...
            existing = (
                await self.db.execute(
                    sa_select_inner(DisputeType).where(
                        DisputeType.reason_name == custom_type_name
                    )
                )
            ).scalar_one_or_none()
            if existing:
                    ...
                    self.db.add(
                        DisputeActivityLog(
                            ...
                            notes=(
                                ...
                                + ", ".join(
                                    f"{d['doc_type']}#{d['doc_id']}" for d in chain
                                )
                            ),
                        )
                    )
    ...
    async def get_enriched_list(
        ...
        from src.schemas.dispute_schemas import (
            AIAnalysisResponse,
            DisputeDetailResponse,
        )
        ...
        analysis_map = {
            ...
            for a in (
                await self.db.execute(
                    sa_select(DisputeAIAnalysis).join(
                        max_a_sq,
                        sa_and_(
        ...
        new_msg_rows = (
            await self.db.execute(
                sa_select(
                    DisputeNewMessage.dispute_id, DisputeNewMessage.has_new_message
                ).where(DisputeNewMessage.dispute_id.in_(id_list))
            )
        ).all()
        ep_actor_map = {
            row.dispute_id: bool(row.has_new_message) for row in new_msg_rows
        }
            ...
            try:
                enriched.append(
                    DisputeDetailResponse(
                        ...
                        assigned_to=active_assign.assignee.email
                        if active_assign
                        else None,
    ...
    async def get_enriched_detail(self, dispute_id: int) -> DisputeDetailResponse:  # noqa: F821  # type: ignore
        ...
        try:
            ...
            has_new = await DisputeNewMessageRepository(self.db).get_for_dispute(
                dispute_id
            )
        except Exception:
            pass
    ...
    async def get_bulk_enriched(
        ...
        max_a_sq = (
            ...
            .where(DisputeAIAnalysis.dispute_id.in_(ids))
            .group_by(DisputeAIAnalysis.dispute_id)
            .subquery()
        )
        analysis_map = {
        ...
        assign_rows = (
            (
                await self.db.execute(
                    ...
                    .where(
                        sa_and_(
                            DisputeAssignment.dispute_id.in_(ids),
                            DisputeAssignment.status == "ACTIVE",
                        )
                    )
                    .order_by(DisputeAssignment.assigned_at.desc())
        ...
        q_count_map = {
            ...
            for row in (
                await self.db.execute(
                    ...
                    .where(
                        sa_and_(
                            DisputeOpenQuestion.dispute_id.in_(ids),
                            DisputeOpenQuestion.status == "PENDING",
                        )
                    )
                    .group_by(DisputeOpenQuestion.dispute_id)
        ...
        ep_actor_map = {
            row.dispute_id: bool(row.has_new_message) for row in new_msg_rows2
        }
...
class ForkRecommendationService:
    ...
    async def dismiss(
        self, dispute_id: int, recommendation_id: int, user_id: int
    ) -> dict:
    ...
    async def _assign_fewest_cases(self, dispute_id: int, prefer_user_id: int) -> None:
        ...
        rows = (
            await self.db.execute(
                ...
                .join(
                    DisputeMaster,
                    DisputeMaster.dispute_id == DisputeAssignment.dispute_id,
                )
                .where(
