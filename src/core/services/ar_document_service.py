...
from src.core.services.key_extraction_service import (
    ...
    try:
        if content_type == "application/pdf" or filename.lower().endswith(".pdf"):
            try:
                ...
                return ""
        if (content_type and content_type.startswith("text/")) or Path(
            filename
        ).suffix.lower() in (".txt", ".csv", ".xml", ".json"):
...
class ARDocumentService:
    ...
    async def get_file_bytes(self, doc_id: int) -> tuple[bytes, str]:
        ...
        safe_name = (
            Path(file_path.split("/")[-1]).name if file_path else f"ar_doc_{doc_id}"
        )
        ...
        if file_path.startswith(GCS_PREFIX):
            ...
            try:
                ...
                logger.error(
                    f"[ar_download] GCS fetch failed for doc_id={doc_id}: {exc}"
                )
                raise FileNotFoundError(
                    f"Could not retrieve file from GCS: {exc}"
                ) from exc
        ...
        raise FileNotFoundError(
            f"Unrecognised file_path format for doc_id={doc_id}: {file_path!r}"
        )

    async def get_signed_url_if_gcs(
        self, doc_id: int, expiry_minutes: int = 30
    ) -> str | None:
        """
    ...
    async def add_manual_key(
        self, doc_id: int, key_type: str, key_value_raw: str, current_user: int
    ) -> dict:
        ...
        return {
            ...
            "key_value_raw": key.key_value_raw,
            "key_value_norm": key.key_value_norm,
    ...
    async def get_document_chain_for_invoice(
        self, invoice_number: str, customer_scope: str
    ) -> list[dict]:
        ...
        try:
            ...
            if inv_key_row:
                ...
                if anchor_doc:
                    inv_key = self._fmt_related(
                        {"document": anchor_doc, "shared_keys": []}
                    )
        except Exception:
            pass
    ...
    async def get_document_chain_for_doc_id(
        self, doc_id: int, customer_scope: str
    ) -> list[dict]:
        """
    ...
    def _fmt_key(self, k) -> dict:
        return {
            "key_id": k.key_id,
            "key_type": k.key_type,
            "key_value_raw": k.key_value_raw,
            "key_value_norm": k.key_value_norm,
            "confidence": round(k.confidence, 2),
            "source": k.source,
            "verified": k.verified,
        }
