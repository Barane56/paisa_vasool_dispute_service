...
class DisputeMaster(Base):
    ...
    email_id = Column(
        Integer, ForeignKey("email_inbox.email_id", ondelete="RESTRICT"), nullable=True
    )  # nullable for FA-created disputes
    invoice_id = Column(
        Integer,
    ...
    parent_dispute_id = Column(
        Integer,
        ForeignKey(
            "dispute_master.dispute_id",
            ondelete="SET NULL",
            use_alter=True,
            name="fk_dispute_master_parent_id",
        ),
        nullable=True,
    )
    created_at = Column(
    ...
    forked_disputes = relationship(
        "DisputeMaster",
        foreign_keys="DisputeMaster.parent_dispute_id",
        lazy="select",
        back_populates="parent_dispute",
    )
    parent_dispute = relationship(
        "DisputeMaster",
        foreign_keys=[parent_dispute_id],
        lazy="select",
        back_populates="forked_disputes",
        remote_side="DisputeMaster.dispute_id",
    )
    relationships_as_source = relationship(
        "DisputeRelationship",
        foreign_keys="DisputeRelationship.source_dispute_id",
        back_populates="source_dispute",
        lazy="select",
        cascade="all, delete-orphan",
    )
    relationships_as_target = relationship(
        "DisputeRelationship",
        foreign_keys="DisputeRelationship.target_dispute_id",
        back_populates="target_dispute",
        lazy="select",
        cascade="all, delete-orphan",
    )
    new_message_flag = relationship(
        "DisputeNewMessage",
        back_populates="dispute",
        lazy="select",
        uselist=False,
    ...
    supporting_documents = relationship(
        ...
        lazy="select",
        cascade="all, delete-orphan",
        order_by="DisputeDocument.created_at",
    )
...
class DisputeRelationship(Base):
    ...
    source_dispute = relationship(
        "DisputeMaster",
        foreign_keys=[source_dispute_id],
        back_populates="relationships_as_source",
        lazy="joined",
    )
    target_dispute = relationship(
        "DisputeMaster",
        foreign_keys=[target_dispute_id],
...
class DisputeNewMessage(Base):
    ...
    dispute = relationship(
        "DisputeMaster", back_populates="new_message_flag", lazy="select"
    )
...
class DisputeDocument(Base):
    ...
    dispute = relationship(
        "DisputeMaster", back_populates="supporting_documents", lazy="select"
    )
...
class DisputeForkRecommendation(Base):
    ...
    new_dispute_id = Column(
        Integer,
        ForeignKey("dispute_master.dispute_id", ondelete="SET NULL"),
        nullable=True,
    )
    actioned_by = Column(
        Integer, ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True
    ...
    new_dispute = relationship(
        "DisputeMaster", foreign_keys=[new_dispute_id], lazy="select"
    )
