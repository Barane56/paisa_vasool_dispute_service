...
class MailboxCredential(Base):
    ...
    fetched_emails = relationship(
        "EmailInboxMessage", back_populates="mailbox", lazy="select"
    )
...
class EmailInboxMessage(Base):
    ...
    mailbox_id = Column(
        Integer,
        ForeignKey("mailbox_credentials.mailbox_id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    dispute_id = Column(
        ...
        ForeignKey(
            ...
            ondelete="SET NULL",
            use_alter=True,
            name="fk_email_msg_dispute_id",
        ),
        nullable=True,
    )
    email_inbox_id = Column(
    ...
    mailbox = relationship(
        "MailboxCredential", back_populates="fetched_emails", lazy="select"
    )
    ...
    email_inbox = relationship(
        "EmailInbox", foreign_keys=[email_inbox_id], lazy="select"
    )
...
class EmailMessageAttachment(Base):
    ...
    message_id = Column(
        Integer,
        ForeignKey("email_inbox_messages.message_id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    message = relationship(
        "EmailInboxMessage", back_populates="attachments", lazy="select"
    )
...
class OutboundEmailAttachment(Base):
    ...
    outbound_id = Column(
        Integer,
        ForeignKey("outbound_emails.outbound_id", ondelete="CASCADE"),
        nullable=False,
    )
    file_name = Column(String(255), nullable=False)
    file_type = Column(String(50), nullable=False)
