...
class ARDocumentKey(Base):
    ...
    doc_id = Column(
        Integer, ForeignKey("ar_documents.doc_id", ondelete="CASCADE"), nullable=False
    )
...
class DisputeARDocument(Base):
    ...
    context_note = Column(
        Text, nullable=True
    )  # e.g. "anchor document" / "graph chain: inv+po match"
    created_at = Column(
        TIMESTAMP(timezone=True), server_default=func.now(), nullable=False
    )
