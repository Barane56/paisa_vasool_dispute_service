...
class EmailRepository(BaseRepository[EmailInbox]):
    ...
    async def update_status(
        self, email_id: int, status: str, failure_reason: str | None = None
    ) -> None:
        values = {"processing_status": status}
        if failure_reason:
            values["failure_reason"] = failure_reason
        await self.db.execute(
            update(EmailInbox).where(EmailInbox.email_id == email_id).values(**values)
        )

    async def get_by_sender(
        self, sender_email: str, limit: int = 20
    ) -> list[EmailInbox]:
        stmt = (
            select(EmailInbox)
