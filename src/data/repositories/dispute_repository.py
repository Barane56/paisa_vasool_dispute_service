...
class DisputeTypeRepository(BaseRepository[DisputeType]):
    ...
    async def get_by_name(self, name: str) -> DisputeType | None:
        result = await self.db.execute(
            select(DisputeType).where(DisputeType.reason_name == name)
        )
        return result.scalar_one_or_none()
...
class DisputeRepository(BaseRepository[DisputeMaster]):
    ...
    async def get_by_id(self, dispute_id: int, **kwargs) -> DisputeMaster | None:
        stmt = (
            ...
            .options(
                ...
                selectinload(DisputeMaster.assignments).selectinload(
                    DisputeAssignment.assignee
                ),
    ...
    async def get_by_dispute_token(self, token: str) -> DisputeMaster | None:
        """
    ...
    async def get_filtered(
        ...
        if search:
            ...
            base_stmt = base_stmt.outerjoin(
                DisputeType,
                DisputeType.dispute_type_id == DisputeMaster.dispute_type_id,
            )
            filters.append(
                or_(
        ...
        stmt = (
            base_stmt.order_by(DisputeMaster.created_at.desc())
            .limit(limit)
            .offset(offset)
        )
...
class DisputeAIAnalysisRepository(BaseRepository[DisputeAIAnalysis]):
    ...
    async def get_latest_for_dispute(self, dispute_id: int) -> DisputeAIAnalysis | None:
        stmt = (
...
class DisputeAssignmentRepository(BaseRepository[DisputeAssignment]):
    ...
    async def has_active_assignment(self, dispute_id: int) -> bool:
        stmt = select(func.count(DisputeAssignment.assignment_id)).where(
            and_(
    ...
    async def get_active_assignment(self, dispute_id: int) -> DisputeAssignment | None:
        stmt = (
...
class AnalysisSupportingRefRepository(BaseRepository[AnalysisSupportingRef]):
    ...
    async def get_by_dispute_via_analysis(
        self, dispute_id: int
    ) -> list[AnalysisSupportingRef]:
        stmt = (
            select(AnalysisSupportingRef)
            .join(
                DisputeAIAnalysis,
                AnalysisSupportingRef.analysis_id == DisputeAIAnalysis.analysis_id,
            )
            .where(DisputeAIAnalysis.dispute_id == dispute_id)
            .order_by(AnalysisSupportingRef.ref_id.asc())
        )
    ...
    async def delete_ref(self, ref_id: int) -> bool:
        ref = (
            await self.db.execute(
                select(AnalysisSupportingRef).where(
                    AnalysisSupportingRef.ref_id == ref_id
                )
            )
        ).scalar_one_or_none()
        if not ref:
...
class DisputeRelationshipRepository(BaseRepository[DisputeRelationship]):
    ...
    async def relationship_exists(self, source_id: int, target_id: int) -> bool:
        stmt = select(DisputeRelationship).where(
            or_(
                and_(
                    DisputeRelationship.source_dispute_id == source_id,
                    DisputeRelationship.target_dispute_id == target_id,
                ),
                and_(
                    DisputeRelationship.source_dispute_id == target_id,
...
class DisputeNewMessageRepository:
    ...
    async def clear_new_message(self, dispute_id: int) -> None:
        ...
        stmt = (
            ...
            .values(has_new_message=False, updated_at=datetime.now(UTC))
        )
        await self.db.execute(stmt)
    ...
    async def get_for_dispute(self, dispute_id: int) -> bool:
        ...
        row = (
            await self.db.execute(
                select(DisputeNewMessage.has_new_message).where(
                    DisputeNewMessage.dispute_id == dispute_id
                )
            )
        ).scalar_one_or_none()
        return bool(row)
