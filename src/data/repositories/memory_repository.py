...
class MemoryEpisodeRepository(BaseRepository[DisputeMemoryEpisode]):
    ...
    async def get_episodes_for_dispute(
        self, dispute_id: int, limit: int = 50
    ) -> list[DisputeMemoryEpisode]:
        stmt = (
        ...
        return [
            {
                ...
                "actor": r["actor"],
                "content_text": r["content_text"][:400],
...
class MemorySummaryRepository(BaseRepository[DisputeMemorySummary]):
    ...
    async def get_for_dispute(self, dispute_id: int) -> DisputeMemorySummary | None:
        result = await self.db.execute(
            select(DisputeMemorySummary).where(
                DisputeMemorySummary.dispute_id == dispute_id
            )
        )
...
class OpenQuestionRepository(BaseRepository[DisputeOpenQuestion]):
    ...
    async def get_pending_for_dispute(
        self, dispute_id: int
    ) -> list[DisputeOpenQuestion]:
        stmt = (
