...
def build_email_processing_graph(db_session=None, llm_client=None):
    ...
    graph.add_node(
        "extract_invoice_data_via_groq",
        partial(node_extract_invoice_data_via_groq, llm_client=llm_client),
    )
    graph.add_node(
        "identify_invoice", partial(node_identify_invoice, db_session=db_session)
    )
    graph.add_node("resolve_token", partial(node_resolve_token, db_session=db_session))
    graph.add_node(
        "pre_fetch_dispute_context",
        partial(node_pre_fetch_dispute_context, db_session=db_session),
    )
    graph.add_node(
        "classify_email",
        partial(node_classify_email, llm_client=llm_client, db_session=db_session),
    )
    graph.add_node("fetch_context", partial(node_fetch_context, db_session=db_session))
    graph.add_node(
        "embed_and_search",
        partial(node_embed_and_search, llm_client=llm_client, db_session=db_session),
    )
    graph.add_node(
        "detect_context_shift",
        partial(
            node_detect_context_shift, llm_client=llm_client, db_session=db_session
        ),
    )
    graph.add_node(
        "resolve_dispute_link",
        partial(node_resolve_dispute_link, db_session=db_session),
    )
    graph.add_node(
        "generate_ai_response",
...
async def run_email_processing(
    ...
    langfuse_context.update_current_trace(
        metadata={
            ...
            "total_disputes": 1
            + len(result.get("inline_dispute_ids") or [])
            + len(result.get("forked_dispute_ids") or []),
